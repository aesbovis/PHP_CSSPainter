...
class css_painter {
  ...
  */
	 ...
	 */
	public function __construct( $img_path, $block_size = 3, $block_blur = 2 ) {
		$this->image_path = $img_path;
		$this->pixel_block_size = $block_size;
		$this->pixel_block_blur = $block_blur;
	...
	protected function _write_css_file() {
		...
		for( $i = 0; $i < $this->pixel_block_X; $i++ ) {
			for( $j = 0; $j < $this->pixel_block_Y; $j++ ) {
				...
				if( 0 == $i && 0== $j ) {
					$css_content .= "\n" . $w . 'px ' . $h . 'px ' . $this->pixel_block_blur . 'px #' . $this->pixel_block_colors[$i][$j];
				} else {
					$css_content .= ",\n" . $w . 'px ' . $h . 'px ' . $this->pixel_block_blur . 'px #' . $this->pixel_block_colors[$i][$j];
				}
			}
